...
final class RegistryGaugeTest extends TestCase
{
    ...
    public function counter(): void
    {
        ...
        self::assertSame([133, 1], array_map(static fn (Gauge $gauge): int => $gauge->gauge(), $counters));
        self::assertSame([['label', 'mushroom'], ['labol', 'mushroom']], array_map(static fn (Gauge $gauge): array => array_map(static fn (Label $label): string => $label->value(), $gauge->labels()), $counters));
        ...
        $counters = [...$gauge->gauges()];
        self::assertCount(2, $counters);
        self::assertSame([132, 64], array_map(static fn (Gauge $gauge): int => $gauge->gauge(), $counters));
        self::assertSame([['label', 'mushroom'], ['labol', 'mushroom']], array_map(static fn (Gauge $gauge): array => array_map(static fn (Label $label): string => $label->value(), $gauge->labels()), $counters));
        ...
        self::assertSame([132, 31, 0], array_map(static fn (Gauge $gauge): int => $gauge->gauge(), $counters));
        self::assertSame([['label', 'mushroom'], ['labol', 'mushroom'], ['labal', 'mushroom']], array_map(static fn (Gauge $gauge): array => array_map(static fn (Label $label): string => $label->value(), $gauge->labels()), $counters));
